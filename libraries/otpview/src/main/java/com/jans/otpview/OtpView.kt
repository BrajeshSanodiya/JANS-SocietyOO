...
import com.jans.otpview.DefaultMovementMethod.Companion.instance
...
class OtpView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = R.attr.otpViewStyle) : AppCompatEditText(context, attrs, defStyleAttr) {
    ...
    private fun setupAnimator() {
        ...
        defaultAddAnimator!!.addUpdateListener {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        height = if (heightMode == MeasureSpec.EXACTLY) heightSize else boxHeight + paddingTop + paddingBottom
    ...
    private fun drawInput(canvas: Canvas, i: Int) { //allows masking for all number keyboard
        if (maskingChar != null &&
    ...
    private fun drawOtpLine(canvas: Canvas, i: Int) {
        ...
        itemLineRect[itemBorderRect.left - halfLineWidth, itemBorderRect.bottom - halfLineWidth, itemBorderRect.right + halfLineWidth] = itemBorderRect.bottom + halfLineWidth
    ...
    private fun drawMaskingText(canvas: Canvas, i: Int, maskingChar: String) {
        ...
        if (rtlTextDirection) {
            ...
            if (reversedCharPosition <= 0 && text != null) {
                drawTextAtBox(canvas, paint, text.toString().replace(".".toRegex(), maskingChar),
    ...
    private fun drawTextAtBox(canvas: Canvas, paint: Paint?, text: CharSequence?, charAt: Int) {
        ...
        canvas.drawText(text.toString(), charAt, charAt + 1, x, y, paint)
    }
    ...
    private fun updateColors() {
        ...
        val color = if (lineColors != null) lineColors!!.getColorForState(drawableState, 0) else currentTextColor
